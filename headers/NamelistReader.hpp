...
namespace NLReader {
    ...
    struct NameList {
        ...
        {
            params.back().values.push_back(word);
        }
    };
    ...
    static inline string get_first_value(string& str, int start) {
        ...
        for (string::size_type i = start; i < str.size(); i++) {
            if (in_dquote == true || in_squote == true) {
                ...
                if (str[i] == '!' || str[i] == ' ' || str[i] == ',' || str[i] == '\t' || str[i] == '\r' || str[i] == '\f' || str[i] == '\v') {
                    // Not in word
                    return str.substr(start, i - start);
                }
            }
    ...
    class NamelistReader {
        ...
        void parse_lines(T& newfile, const string& filename) {
            ...
            while (getline(newfile, line)) { //read data from file object and put it into string.
                ...
                if (line[0] >= 48 && line[0] <= 57 || line[0] == '-') expect = ValueOrEOL;
    ...
    public:
        ...
        void add_namelist(const string& namelist) {
            if (modifiable) {
                namelists.push_back(NLReader::NameList(namelist));
            }
            else {
